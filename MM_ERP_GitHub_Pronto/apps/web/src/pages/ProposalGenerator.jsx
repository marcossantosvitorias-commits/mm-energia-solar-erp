...
function cabecalho(doc, titulo, destaque, subtitulo, selo) {
  ...
  doc.setFontSize(7.5); doc.roundedRect(157, 10, 39, 9, 4, 4, 'S'); doc.text(selo, 176.5, 15.7, { align: 'center' });
  doc.setFontSize(21); doc.text(titulo, 12, 33); doc.setTextColor(255, 194, 15); doc.text(destaque, 12, 43);
  doc.setTextColor(225, 234, 244); doc.setFont('helvetica', 'normal'); doc.setFontSize(9.5); doc.text(doc.splitTextToSize(subtitulo, 178), 12, 53);
}
...
function desenhoPainel(doc, x, y, w, h) {
  doc.setFillColor(18, 64, 105); doc.roundedRect(x, y, w, h, 2, 2, 'F'); doc.setDrawColor(155, 196, 230);
  for (let i = 1; i < 6; i += 1) doc.line(x + (w / 6) * i, y, x + (w / 6) * i, y + h);
  for (let i = 1; i < 4; i += 1) doc.line(x, y + (h / 4) * i, x + w, y + (h / 4) * i);
}
function desenhoMicro(doc, x, y, w, h) {
  doc.setFillColor(242, 244, 247); doc.setDrawColor(140, 148, 158); doc.roundedRect(x, y, w, h, 5, 5, 'FD');
  doc.setFillColor(24, 33, 45); doc.roundedRect(x + 8, y + 7, w - 16, h - 14, 3, 3, 'F');
  doc.setTextColor(255, 255, 255); doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.text('MICROINVERSOR', x + w / 2, y + h / 2 + 2, { align: 'center' });
}
...
export default function ProposalGenerator({ quantidadePlacas, precoRecomendado, modulo, inversor, potenciaSistemaKw, precoCartao = 0, taxaCartao = 0 }) {
  ...
  const [dados, setDados] = useState({ cliente: '', cidade: 'Bauru/SP', telefone: '', potenciaPlaca: potenciaInicial, marcaPlaca: modulo || 'Painel solar', inversor: inversor || 'Microinversor solar', geracaoMensal: Math.round(gerarPorPainel(potenciaInicial) * quantidadePlacas), valorProposta: Number(precoRecomendado || 0).toFixed(2), validade: 7, observacoes: 'Projeto executivo, instalação, homologação junto à concessionária, estrutura, proteções elétricas e pós-venda inclusos.' });
  ...
  const carregarDados = async () => {
    ...
    } catch { /* mantém o gerador disponível */ }
  };
  ...
  const validarNome = () => { if (dados.cliente.trim()) return true; setMensagem('Informe o nome do cliente antes de gerar a proposta.'); document.querySelector('input[name="cliente"]')?.focus(); return false; };
  const salvarProposta = async (status = 'Gerada') => {
    ...
    setSalvando(true); try { const { data, error } = await supabase.from('sales_proposals').insert(payload(status)).select('*').single(); if (error) return { saved: false, warning: `O PDF foi gerado, mas a proposta não foi registrada no CRM: ${error.message}` }; if (clienteId) { try { await createClientInteraction(clienteId, { type: 'proposta', description: `Proposta ${status.toLowerCase()} no valor de ${moeda.format(valor)}.`, nextActionAt: '' }); } catch {} } await carregarDados(); return { saved: true, data }; } catch (error) { return { saved: false, warning: `O PDF foi gerado, mas a proposta não foi registrada no CRM: ${error?.message || 'erro inesperado'}` }; } finally { setSalvando(false); }
  };
  ...
  const criarArquivoPdf = (origem = dados) => {
    ...
    doc.addPage(); cabecalho(doc, 'Escolha a melhor forma de', 'realizar seu projeto.', 'Compare financiamento BelCred, cartão de crédito e pagamento à vista.', 'CONDIÇÕES DE PAGAMENTO');
    ...
    doc.addPage(); cabecalho(doc, 'Equipamentos escolhidos para', 'desempenho e segurança.', 'Componentes principais previstos para o sistema fotovoltaico desta proposta.', 'EQUIPAMENTOS');
    arredondar(doc,12,84,90,105,[255,255,255]); desenhoPainel(doc,24,94,66,42); doc.setTextColor(16,47,82); doc.setFont('helvetica','bold'); doc.setFontSize(11); doc.text('Painel fotovoltaico',16,146); doc.setFontSize(8.3); doc.text(doc.splitTextToSize(origem.marcaPlaca || 'Painel fotovoltaico',80),16,154); doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.text(`• Potência: ${potenciaPainel} W`,16,169); doc.text(`• Quantidade: ${qtd} unidades`,16,175); doc.text('• Tecnologia bifacial N-Type',16,181);
    arredondar(doc,108,84,90,105,[255,255,255]); desenhoMicro(doc,120,94,66,42); doc.setTextColor(16,47,82); doc.setFont('helvetica','bold'); doc.setFontSize(11); doc.text('Microinversor',112,146); doc.setFontSize(8.3); doc.text(doc.splitTextToSize(origem.inversor || 'Microinversor solar',80),112,154); doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.text('• Monitoramento individual',112,169); doc.text('• Conversão e proteção',112,175); doc.text('• Configuração inclusa',112,181);
    doc.setFont('helvetica','bold'); doc.setFontSize(12); doc.text('Garantias',12,204); [['PAINÉIS', '15 anos de garantia'], ['MICROINVERSOR', '15 anos de garantia'], ['INSTALAÇÃO', '1 ano de garantia']].forEach(([a,b],i)=>{ const x=12+i*63; arredondar(doc,x,211,59,30,i===1?[234,244,255]:[248,249,252],i===1?[34,112,181]:[220,226,235]); doc.setTextColor(95,109,128); doc.setFontSize(6.5); doc.text(a,x+4,219); doc.setTextColor(16,47,82); doc.setFontSize(9); doc.text(b,x+4,230); });
    arredondar(doc,12,249,186,14,[255,251,230],[235,203,74]); doc.setTextColor(80,69,24); doc.setFontSize(6.8); doc.text(doc.splitTextToSize('As garantias dos equipamentos seguem as condições do fabricante. A garantia da instalação cobre os serviços executados pela MM Energia Solar.',176),17,255); rodape(doc, validade);

    doc.addPage(); cabecalho(doc, 'Seu projeto acompanhado', 'do início ao pós-venda.', 'Um processo organizado para entregar segurança, qualidade e tranquilidade.', 'ETAPAS DO PROJETO');
  ...
  const gerarESalvar = async () => { if (!validarNome()) return; const arquivo = criarArquivoPdf(); baixarPdf(arquivo); const resultado = await salvarProposta('Gerada'); setMensagem(resultado.saved ? 'Proposta profissional salva e PDF gerado.' : resultado.warning); };
  const enviarWhatsApp = async () => { if (!validarNome()) return; const arquivo = criarArquivoPdf(); try { if (!navigator.share || !navigator.canShare?.({ files: [arquivo] })) { baixarPdf(arquivo); setMensagem('O PDF foi baixado. Anexe-o no WhatsApp Business pelo botão de clipe.'); return; } await navigator.share({ files: [arquivo], title: `Proposta MM Energia Solar - ${dados.cliente.trim()}`, text: `Olá, ${dados.cliente.trim()}! Segue em anexo sua proposta da MM Energia Solar.` }); const resultado = await salvarProposta('Enviada'); setMensagem(resultado.saved ? 'PDF compartilhado e proposta registrada como enviada.' : resultado.warning); } catch (error) { if (error?.name === 'AbortError') { setMensagem('Compartilhamento cancelado.'); return; } baixarPdf(arquivo); setMensagem('Não foi possível compartilhar automaticamente. O PDF foi baixado.'); } };
  ...
  return <section className="finance-panel">
    <div className="finance-panel-header proposal-no-print"><div><h2>Proposta profissional ERP 2.0</h2><p>PDF premium com equipamentos, garantias, BelCred, cartão e compartilhamento no WhatsApp Business.</p></div></div>
    ...
    <div className="proposal-payment-grid"><div className="proposal-payment-card"><span>Financiamento BelCred</span><strong>{paymentOptions.belcred.installments}x de {moeda.format(paymentOptions.belcred.installmentValue)}</strong><select className="proposal-no-print" value={planoBelcred} onChange={(event) => setPlanoBelcred(Number(event.target.value))} style={{ width: '100%', marginTop: 10 }}>{belcred.map((item) => <option value={item.parcelas} key={item.parcelas}>{item.parcelas}x de {moeda.format(item.valor)}</option>)}</select></div><div className="proposal-payment-card"><span>Cartão de crédito</span><div className="proposal-card-options">{opcoesCartao.map((item) => <div className="proposal-card-option" key={item.parcelas}>{item.parcelas}x de {moeda.format(item.valorParcela)}</div>)}</div></div></div>
    ...
    <div className="proposal-no-print" style={{ marginTop: 18, border: '1px solid #dce5ef', borderRadius: 18, padding: 16 }}><div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}><Sparkles size={20} /><strong>Proposta pronta</strong></div><div className="finance-actions"><button className="finance-button" type="button" disabled={salvando} onClick={gerarESalvar}><FileDown size={20} /> Salvar e gerar PDF profissional</button><button className="finance-button" type="button" disabled={salvando} onClick={enviarWhatsApp}><MessageCircle size={20} /> Compartilhar PDF no WhatsApp Business</button></div><div style={{ marginTop: 10, color: '#667085', fontSize: 12 }}><ShieldCheck size={15} /> PDF comercial em 4 páginas com sistema, pagamentos, equipamentos, garantias e etapas.</div></div>
