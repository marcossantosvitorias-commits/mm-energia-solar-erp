...
function PrecificacaoKitsPage() {
  ...
  const resultado = useMemo(() => {
    ...
    const custoEquipamentos = custoPaineis + numero(form.custoInversor) + numero(form.custoEstrutura);
    ...
    const aliquotaTributo = modoTributario === 'atual' ? percentual(form.impostoVenda) : percentual(form.ibsCbs);
    ...
    const creditoTributario = modoTributario === 'reforma' ? baseCredito * percentual(form.creditoCompras) : 0;
    const divisor = 1 - aliquotaTributo - aliquotaComissao - margem;
    const precoCalculado = divisor > 0 ? (custoTotal - creditoTributario) / divisor : 0;
    ...
    const valorLiquidoRecebido = modoTributario === 'reforma' ? precoVenda - tributoLiquido : precoVenda;
    const precoComDesconto = precoVenda * (1 - percentual(form.descontoMaximo));
    const markup = custoTotal > 0 ? precoVenda / custoTotal : 0;

    return {
      custoPaineis, custoEquipamentos, custosOperacionais, custoTotal, baseCredito,
      creditoTributario, precoVenda, precoCartao, valorProposta, tributoBruto,
      tributoLiquido, comissao, lucro, margemReal, valorLiquidoRecebido,
      precoComDesconto, markup,
    };
  }, [form, quantidadePlacas, modoTributario, formaPagamento]);

  const comparativo = useMemo(() => presets.map((quantidade) => {
    const custoPaineis = quantidade * numero(form.custoPlaca);
    const custoTotal = custoPaineis + numero(form.custoInversor) + numero(form.custoEstrutura) +
      numero(form.materialEletrico) + numero(form.frete) + numero(form.maoDeObra) +
      numero(form.engenharia) + numero(form.trt) + numero(form.combustivel) + numero(form.outrosCustos);
    const aliquotaTributo = modoTributario === 'atual' ? percentual(form.impostoVenda) : percentual(form.ibsCbs);
    const credito = modoTributario === 'reforma'
      ? (custoPaineis + numero(form.custoInversor) + numero(form.custoEstrutura)) * percentual(form.baseCredito) * percentual(form.creditoCompras)
      : 0;
    const divisor = 1 - aliquotaTributo - percentual(form.comissao) - percentual(form.margemDesejada);
    const precoCalculado = divisor > 0 ? (custoTotal - credito) / divisor : 0;
    const preco = precosAVista[quantidade] ?? precoCalculado;
    const precoCartao = preco / (1 - TAXA_CARTAO_12X / 100);
    return { quantidade, custoTotal, credito, preco, precoCartao, parcela12: precoCartao / 12 };
  }), [form, modoTributario]);
  ...
  return (
    <FinanceLayout title="Formação de preço dos kits" subtitle="Escolha o kit, a forma de pagamento e envie o valor correto para a proposta." theme="empresa">
      <section className="finance-panel">
        <div className="finance-panel-header"><h2>Escolha o tamanho do kit</h2></div>
        <div className="kit-preset-grid">
          {presets.map((quantidade) => (
            <button key={quantidade} className={`kit-preset ${quantidadePlacas === quantidade ? 'active' : ''}`} onClick={() => selecionarKit(quantidade)}>
              ...
              {precosAVista[quantidade] && <small>{formatarMoeda(precosAVista[quantidade])} à vista</small>}
            </button>
          ))}
      ...
      <section className="finance-panel">
        <div className="finance-panel-header"><div><h2>Forma de pagamento da proposta</h2><p>Escolha qual valor será levado para o PDF e para o WhatsApp.</p></div></div>
      ...
      <section className="finance-panel">
        <div className="finance-panel-header"><h2>Regime de cálculo</h2></div>
        <div className="tax-mode-grid">
          <button className={modoTributario === 'atual' ? 'active' : ''} onClick={() => setModoTributario('atual')}><strong>Modelo atual</strong><span>Simples Nacional ou imposto atual</span></button>
          <button className={modoTributario === 'reforma' ? 'active' : ''} onClick={() => setModoTributario('reforma')}><strong>IBS + CBS</strong><span>Com crédito tributário e split payment</span></button>
        </div>
      </section>

      <section className="finance-two-columns">
        <article className="finance-panel"><h2>Custos do kit</h2><div className="finance-form">
        ...
        <article className="finance-panel"><h2>Tributos e margem</h2><div className="finance-form">
        ...
        </div><div className="tax-warning">As alíquotas ficam editáveis para você ajustar conforme a orientação do contador.</div></article>
      </section>
      ...
      <section className="finance-grid">
        ...
        <StatCard label="Preço no cartão" value={formatarMoeda(resultado.precoCartao)} helper={`12x de ${formatarMoeda(resultado.precoCartao / 12)}`} tone="warning" />
      ...
      <ProposalGenerator key={`${quantidadePlacas}-${resultado.valorProposta.toFixed(2)}-${formaPagamento}`} quantidadePlacas={quantidadePlacas} precoRecomendado={resultado.valorProposta} />

      <section className="finance-two-columns">
        <article className="finance-panel"><h2>Detalhamento do cálculo</h2>
        ...
        <article className="finance-panel"><h2>Informações comerciais</h2>
          ...
          <div className="finance-list-item"><div><strong>Preço à vista</strong><span>Pagamento sem taxa de cartão</span></div><strong>{formatarMoeda(resultado.precoVenda)}</strong></div>
          <div className="finance-list-item"><div><strong>Cartão em 12x</strong><span>Taxa de {TAXA_CARTAO_12X}% incluída</span></div><strong>12x de {formatarMoeda(resultado.precoCartao / 12)}</strong></div>
          <div className="pricing-highlight"><span>Selecionado para a proposta</span><strong>{formaPagamento === 'cartao' ? 'Cartão' : 'À vista'} • {formatarMoeda(resultado.valorProposta)}</strong></div>
        </article>
      </section>

      <section className="finance-panel">
        <div className="finance-panel-header"><h2>Tabela automática de preços</h2></div>
        <div className="finance-table-wrapper"><table className="finance-table">
          <thead><tr><th>Kit</th><th>Custo total</th><th>Preço à vista</th><th>Total no cartão</th><th>Cartão 12x</th></tr></thead>
          <tbody>{comparativo.map((item) => (
            <tr key={item.quantidade} onClick={() => selecionarKit(item.quantidade)} style={{ cursor: 'pointer' }}>
              <td><strong>{item.quantidade} placas</strong></td><td>{formatarMoeda(item.custoTotal)}</td>
              <td><strong>{formatarMoeda(item.preco)}</strong></td><td>{formatarMoeda(item.precoCartao)}</td><td>{formatarMoeda(item.parcela12)}</td>
            </tr>
          ))}</tbody>
        </table></div>
      </section>
