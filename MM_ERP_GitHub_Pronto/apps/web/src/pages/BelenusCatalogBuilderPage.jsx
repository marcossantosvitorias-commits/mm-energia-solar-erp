...
const STEPS = [
  ...
  { id: 'inversor', title: 'Inversor ou microinversor', subtitle: 'Escolha o equipamento e a quantidade', icon: Zap },
  { id: 'estrutura', title: 'Estrutura', subtitle: 'Perfis, grampos, hastes e suportes', icon: Settings2 },
  { id: 'cabos', title: 'Cabos', subtitle: 'Escolha o cabo e informe a metragem', icon: Cable },
...
function unidadeLabel(item) {
  const unit = String(item.unit || 'UN').toUpperCase();
  if (unit === 'M') return 'metros';
  if (unit === 'JG') return 'jogos';
  if (unit === 'PT') return 'pacotes';
  return 'unidades';
}
...
export default function BelenusCatalogBuilderPage() {
  ...
  const catalogoEtapa = useMemo(() => {
    ...
    return catalogoCompleto
      ...
      .sort((a, b) => numero(a.price) - numero(b.price));
  }, [catalogoCompleto, etapaAberta, busca]);

  const quantidadePadrao = (item) => {
    if (quantidades[item.sku] != null) return Math.max(1, numero(quantidades[item.sku]));
    if (item.category === 'Módulo') return Math.max(1, numero(placasDesejadas));
    return 1;
  };

  const definirQuantidade = (item, valor) => {
    const quantidade = Math.max(1, numero(valor));
    setQuantidades((atual) => ({ ...atual, [item.sku]: quantidade }));
  };

  const adicionar = (item) => {
    const quantidade = quantidadePadrao(item);
    setMensagem('');
    setItens((atual) => {
      const existente = atual.find((x) => x.sku === item.sku);
      if (existente) {
        return atual.map((x) => (x.sku === item.sku ? { ...x, quantidade } : x));
      }
  ...
  const alterarQtd = (sku, quantidade) => setItens((atual) => atual.map((x) => (
    x.sku === sku ? { ...x, quantidade: Math.max(1, numero(quantidade)) } : x
  )));
  const remover = (sku) => {
    setMensagem('');
    setItens((atual) => atual.filter((x) => x.sku !== sku));
  };
  ...
  return (
    <FinanceLayout title="Cotações Belenus" subtitle="Monte o kit por etapas com os preços Belenus já com 12% de desconto.">
      <div className="bel-builder-page">
        <section className="finance-panel bel-start-card">
          ...
          <p>Digite a quantidade de placas e depois escolha cada componente com a quantidade necessária.</p>
          <label className="bel-panels-input">
            <span>Quantidade de placas</span>
            <div>
              <input type="number" min="1" step="1" value={placasDesejadas} onChange={(e) => setPlacasDesejadas(Math.max(1, numero(e.target.value)))} />
        ...
        <section className="bel-steps">
          {STEPS.map((step) => {
            ...
            return (
              <div className="bel-step" key={step.id}>
                ...
                {aberto && (
                  <div className="bel-step-content">
                    ...
                    <div className="bel-products-list">
                      {catalogoEtapa.map((item) => {
                        ...
                        const qtd = quantidadePadrao(item);
                        return (
                          <article className={`bel-product-row ${selecionado ? 'selected' : ''}`} key={item.sku}>
                            <div className="bel-product-info">
                              <strong>{item.brand} · {item.model}</strong>
                              <small>{item.sku}</small>
                              <span>{moeda.format(item.price)} <em>com -12%</em></span>
                            </div>
                            <label className="bel-item-qty">
                              <span>Quantidade</span>
                              <div>
                                <button type="button" onClick={() => definirQuantidade(item, Math.max(1, qtd - 1))}>−</button>
                                <input type="number" min="1" step="1" value={qtd} onChange={(e) => definirQuantidade(item, e.target.value)} />
                                <button type="button" onClick={() => definirQuantidade(item, qtd + 1)}>+</button>
                              </div>
                              <small>{unidadeLabel(item)}</small>
                            </label>
                            <button
                              type="button"
                              className={`bel-add-button ${selecionado ? 'remove' : ''}`}
                              onClick={() => (selecionado ? remover(item.sku) : adicionar(item))}
                            >
                              {selecionado ? <><Trash2 size={16} /> Remover</> : <><PackagePlus size={16} /> Adicionar</>}
                            </button>
                          </article>
                        );
                      })}
        ...
        <section className="finance-panel bel-summary">
          <div className="finance-panel-header"><div><h2><ShoppingCart size={20} /> Kit montado</h2><p>As quantidades podem ser alteradas novamente aqui antes de enviar.</p></div></div>
          {!itens.length ? <div className="finance-empty">Escolha os componentes nos botões acima.</div> : (
            <div className="bel-cart-list">
              {itens.map((item) => (
                <div className="bel-cart-row" key={item.sku}>
