...
export function AuthProvider({ children }) {
  ...
  const login = async ({ email, password }) => {
    if (!isSupabaseConfigured) {
      return { ok: false, message: 'Supabase não configurado. O acesso local foi desativado.' };
    }
  ...
  const value = useMemo(() => ({
    user, loading, isAuthenticated: Boolean(user), isDemoMode: false, login, logout, hasRole,
