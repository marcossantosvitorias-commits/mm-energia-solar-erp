...
export default function HybridPresetProposal() {
  ...
  const generatePdf = async () => {
    ...
    try {
      // Carrega a biblioteca só quando o usuário pede o PDF. Não converte imagens
      // grandes no celular, evitando o travamento observado no Android.
      ...
      doc.setFontSize(15);
      doc.text('MM ENERGIA SOLAR', 12, 16);
      doc.setFontSize(18);
      ...
      doc.roundedRect(10, y, 190, 46, 3, 3, 'FD');
      doc.setTextColor(...navy);
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(12);
      ...
      doc.text(`Potência instalada: ${KIT.systemPowerKw.toFixed(2).replace('.', ',')} kWp`, 15, y + 37);
      doc.text('Tensão: Mono 220V', 105, y + 37);
      ...
      rows.slice(0, 10).forEach((fee, index) => {
        ...
        doc.setFont('helvetica', 'bold');
        doc.text(`${installments}x`, 14, rowY + 4.3);
        doc.setFont('helvetica', 'normal');
        doc.text(`Taxa ${Number(fee.fee_percent).toFixed(2).replace('.', ',')}%`, 31, rowY + 4.3);
        doc.text(`Total ${money(gross)}`, 82, rowY + 4.3);
        doc.setFont('helvetica', 'bold');
        doc.text(`${installments}x de ${money(installment)}`, 195, rowY + 4.3, { align: 'right' });
      });
