...
async function validToken(supplied: string) {
  ...
  const { data, error } = await supabase.rpc("verify_whatsapp_webhook_token", { p_token: supplied });
...
function normalizeGeneric(payload: any) {
  ...
  const body = firstText(msg.body, typeof msg.text === "string" ? msg.text : undefined, msg.message, msg.content, msg.text?.body, media.mediaCaption, payload.body, typeof payload.text === "string" ? payload.text : undefined);
  ...
  return [{
    ...
    externalMessageId: firstText(msg.id, msg.message_id, msg.messageId, payload.message_id, payload.messageId),
    phone,
    contactName: firstText(payload.contact?.name, payload.name, msg.contact_name, msg.contactName),
    direction,
    senderType: detectSenderType(payload, direction),
    messageType,
    body,
    occurredAt,
    source: firstText(payload.source, payload.provider, payload.platform) || "whatsapp_generic",
    raw: payload,
    ...media,
  }];
}
...
Deno.serve(async (req: Request) => {
  ...
  const items = isMeta ? normalizeMeta(payload) : normalizeGeneric(payload);
  const processed: any[] = [];
  ...
  for (const item of items) {
    ...
    const conversationPatch: any = {
      phone: item.phone,
      contact_name: item.contactName || existing?.contact_name || null,
      external_conversation_id: item.externalConversationId || existing?.external_conversation_id || null,
      source: item.source,
      status,
      needs_reply: needsReply,
      last_message_at: item.occurredAt,
      last_message_preview: preview,
      updated_at: new Date().toISOString(),
    };
    if (inbound) {
      conversationPatch.last_inbound_at = item.occurredAt;
      conversationPatch.unread_count = (existing?.unread_count ?? 0) + 1;
    }
    if (item.direction === "outbound") {
      conversationPatch.last_outbound_at = item.occurredAt;
      if (humanOutbound) conversationPatch.unread_count = 0;
    }
